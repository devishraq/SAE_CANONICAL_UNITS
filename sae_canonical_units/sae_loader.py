...
def load_sae(release, sae_id):
    ...
    sae = sae.to(DEVICE).to(torch.float32) # Force float32 to prevent math bombs
    sae.eval()
...
def load_gemma_9b_saes():
    small = load_sae("gemma-scope-9b-it-res-canonical", "layer_20/width_16k/canonical")
    large = load_sae("gemma-scope-9b-it-res-canonical", "layer_20/width_131k/canonical")
    return small, large
